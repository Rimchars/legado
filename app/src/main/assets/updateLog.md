...
**2026/08/17**
- 新增 SMB 网络文件支持：jcifs 远程书库/压缩包漫画免下载直读/上传下载
- WebDav 远程 zip 漫画免下载直读，加入书架秒完成，打开即看
- 漫画底部菜单对齐小说：章节条/亮度行/快捷面板统一 Compose 样式
- 漫画自动翻页控制条、设置弹窗（屏幕方向/保持亮屏/亮度控件/进度条行为等）
- 漫画顶栏三点菜单同小说（书签/预下载/自动翻页/滤镜等），书签弹窗漫画版只留内容框
- 网页服务完整同步：压缩包漫画图片直读、前端与 modules/web 源码对齐、漫画面板贴左
- 远程图书文案统一（地址/账号/密码/目录），修复无服务器配置时反复弹初始化错误
- 代码编辑器默认主题改为跟随系统 + 现代浅色
